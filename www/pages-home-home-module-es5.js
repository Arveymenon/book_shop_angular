...
var HomePage = /** @class */ (function () {
    function HomePage(router, locationService, storage, loading, http, cartService) {
        var _this = this;
        this.router = router;
        ...
        this.loading.create();
        this.storage.get('default_address').then(function (address) {
            console.log(address);
            if (address || address != null) {
                console.log('address found');
                _this.locationService.getLocationByCoordinates(address.lat, address.lng).then(function (res) {
                    console.log(res);
        ...
        this.storage.get('selectedFilters').then(function (selectedFilters) {
            console.log(selectedFilters);
            if (selectedFilters) {
                selectedFilters.subject || selectedFilters.board || selectedFilters.standard || selectedFilters.refurbished ?
                    _this.showFilters = false : _this.showFilters = true;
                _this.selectedFilters = selectedFilters;
                _this.getBooks();
                _this.subject.setValue(selectedFilters.board);
                _this.standard.setValue(selectedFilters.standard);
                _this.board.setValue(selectedFilters.board);
                _this.refurbished.setValue(selectedFilters.refurbished);
            }
            _this.http.call('get', 'api/books/filter/options').then(function (res) {
                console.log(res);
                _this.filters = res.response;
                _this.subject.valueChanges.subscribe(function (subject) { _this.selectedFilters.subject = subject; _this.getBooks(); });
                _this.standard.valueChanges.subscribe(function (standard) { _this.selectedFilters.standard = standard; _this.getBooks(); });
                _this.board.valueChanges.subscribe(function (board) { _this.selectedFilters.board = board; _this.getBooks(); });
                _this.refurbished.valueChanges.subscribe(function (refurbished) {
                    _this.selectedFilters.refurbished = refurbished;
                    _this.getBooks();
                });
                console.log(_this.selectedFilters);
            });
        });
    }
    ...
    HomePage.prototype.getBooks = function () {
        ...
        this.cartService.getBooks(this.selectedFilters).then(function (res) {
            console.log(res);
            console.log(_this.cartService.books);
            _this.cartService.calculate();
        });
    };
    HomePage.prototype.cart = function () {
        this.router.navigate(['cart']);
    };
    HomePage.prototype.addToCart = function (bookId) {
        console.log(bookId);
        console.log(this.cartService.books);
        this.cartService.addProduct(bookId, 1).then(function (res) {
            console.log(res);
        });
    };
    HomePage.prototype.removeFromCart = function (bookId) {
        console.log(bookId);
        console.log(this.cartService.books);
        var book = this.cartService.books.findIndex(function (o) { return o.id == bookId; });
        console.log(book);
        this.cartService.removeProduct(bookId, 1).then(function (res) {
            console.log(res);
            console.log(book);
    ...
    HomePage.prototype.refurbishedToggle = function (itt) {
        var _this = this;
        console.log(itt);
        this.cartService.refurbishedToggle(this.cartService.books[itt].id).then(function (res) {
            console.log(res);
            var cartItt = _this.cartService.cart.findIndex(function (o) { return o.id == _this.cartService.books[itt].id; });
            if (cartItt > -1) {
                _this.cartService.cart[cartItt].refurbished = _this.cartService.cart[cartItt].refurbished == true ? false : true;
                _this.cartService.calculate();
            }
        });
    };
    HomePage.prototype.refurbishedOnly = function () {
        this.getBooks();
    };
